...
// Forces outbound connections to prefer IPv4 over IPv6. Without this, some
// cloud servers try IPv6 first for services like Gmail's SMTP, hit a dead
// end (IPv6 not fully routed), and fail with ENETUNREACH.
...
// ==== EMAIL SETUP (for password reset emails) ====
const EMAIL_ADDRESS = 'lasna2026@gmail.com';
...
// ==== ACCOUNTS: FORGOT PASSWORD (generates a reset link and emails it) ====
app.post('/api/forgot-password', async (req, res) => {
  ...
  // Always respond with success, even if the email isn't found - this prevents
  // someone from using this form to check which emails have LASNA accounts.
  if (!user) {
  ...
  db.prepare('INSERT INTO password_resets (token, email, expires_at) VALUES (?, ?, ?)').run(token, email, expiresAt);

  const resetLink = 'https://hellolasna.com/?reset=' + token;

  try {
    await transporter.sendMail({
      from: EMAIL_ADDRESS,
      to: email,
      subject: 'Reset your LASNA password',
      html: '<p>Someone requested a password reset for your LASNA account.</p>' +
...
// ==== ACCOUNTS: RESET PASSWORD (using the token from the emailed link) ====
app.post('/api/reset-password', async (req, res) => {
  ...
  db.prepare('UPDATE users SET password_hash = ? WHERE email = ?').run(passwordHash, resetRow.email);
  db.prepare('DELETE FROM password_resets WHERE token = ?').run(token);
...
// ==== ADMIN: DASHBOARD STATS (restricted to owner's email only) ====
const ADMIN_EMAIL = 'lasna2026@gmail.com';
...
app.get('/api/admin/stats', (req, res) => {
  ...
  const totalUsers = db.prepare('SELECT COUNT(*) as count FROM users WHERE password_hash IS NOT NULL').get().count;
  const subscribedUsers = db.prepare('SELECT COUNT(*) as count FROM users WHERE is_subscribed = 1').get().count;
  const authorCount = db.prepare("SELECT COUNT(*) as count FROM users WHERE user_type = 'author'").get().count;
  const studentCount = db.prepare("SELECT COUNT(*) as count FROM users WHERE user_type = 'student'").get().count;
  ...
  const totalWorldEntries = db.prepare('SELECT COUNT(*) as count FROM world_entries').get().count;
  const estimatedMonthlyRevenue = (subscribedUsers * 1.99).toFixed(2);

  const recentUsers = db.prepare(`SELECT email, user_type, is_subscribed, word_count FROM users
    WHERE password_hash IS NOT NULL ORDER BY rowid DESC LIMIT 10`).all();
  ...
  res.json({
    ...
    totalWorldEntries: totalWorldEntries,
